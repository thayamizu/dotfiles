...
(defsubst hatena-d-api-id-info (id)
  ;; diary user date epoch
  (split-string (or (car-safe (last (split-string id ":"))) "") "-"))
...
(defun hatena-d-api-entry-member-uri (entry)
  (destructuring-bind (diary user date epoch)
      (hatena-d-api-id-info (hatena:d:entry-id entry))
    (let ((type (hatena:d:entry-type entry))
          (user (hatena:d:entry-user entry)))
